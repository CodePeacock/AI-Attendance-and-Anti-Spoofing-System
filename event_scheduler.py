...
def registered_vs_absent_staffs(all_staffs):
    ...
    if len(output) != 0:
        ...
        absent_staff_ids = [x for x in all_staff_ids if x not in registered_staff_ids]
    else:
...
def absent_emails():
    ...
    return [all_staffs[item] for item in absent_staff_ids]
...
def get_manager_email():
    ...
    cur = conn.cursor()
    cur.execute(
        "select email_address from attendance where department=%s ", (department)
    )
    data = cur.fetchall()
    if len(data) != 0:
...
def generate_attendance_sheet():
    dt = datetime.now()
    date = str(dt).split(" ")[0]
    csv_name = f"Attendance_Details/attendance_{date}.csv"
    ...
    if len(mydata) < 1:
        ...
        mark_attendance_obj.write_csv_header(
            id="Id", staff_name="Staff_Name", date="Date", time="Time", status="Status"
        )
        for items in mydata:
...
def send_mail():
    ...
    for email in absent_staff_emails:
        server.sendmail("enter your mail", email, "You are absent")
    ...
    msg.add_attachment(
        file_data, maintype="file", subtype=file_type, filename=file_name
    )
